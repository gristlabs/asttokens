...
def to_source(node):
  ...
  if isinstance(node, ast.Module):
    ...
    # We also need to make a deep copy of node (not sure why, but node seems affected by the
    # astroid TreeRebuilder.
...
class MarkChecker(object):
  ...
  def verify_all_nodes(self, test_case):
    # A generic way to test atok.get_text() on the ast tree: for each statement and expression
